...
def GetTextAndTagFromUsersTag(users):    
    for users in queue.findall('users'):
        user = users[0]
...
def writeRowIntoSchedulerReport(pathInsrt):
    ...
    # or this method is not good!!. 
    

    
    rowAppToReport.insert(0, dtTimeStamp)

    header = open("/home/kaoper/reportOutput/logYarnResource_"+str(dateF)+"_SchedulerAPI.csv").readline().rstrip('\n')
    lstHeader = header.split(',')
    lstHeader.insert(0, 'run_timestamp')

    reportH = open("/home/kaoper/reportOutput/logYarnResource_"+str(dateF)+"_SchedulerAPI.csv", 'w')
    csvwriter = csv.writer(reportH)
    csvwriter.writerow(lstHeader)
    reportH.close()
    ...
    print(" *********** Complete Write SchedulerAPI Report File ***********")
...
# copy header longest to set be header report file.

src = "/home/kaoper/tmp/fileMaster/fileMaster.csv"
dst = "/home/kaoper/reportOutput/logYarnResource_"+str(dateF)+"_SchedulerAPI.csv"
copyfile(src, dst)
